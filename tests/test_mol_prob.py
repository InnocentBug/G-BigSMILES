...
def test_mol_prob():
    test_objects = [
        ...
        (
            "OCC{[<][<]C(N)C[>], [<]CC(C(=O)C[<])[>] ;[H][>] [>]}|gauss(500, 50)|CC[Si]",
            0.007696281511746323,
            2,
        ),
    ]
    ...
    for big_smi, prob, num_matches in test_objects:
        ...
        print(big_smi, prob, calc_prob, [m.probability for m in matches])
        ...
        for i in range(6):
            ...
            print(i, smi)
            calc_prob, matches = bigsmiles_gen.mol_prob.get_ensemble_prob(smi, mol)
            print(i, calc_prob, [m.probability for m in matches])
